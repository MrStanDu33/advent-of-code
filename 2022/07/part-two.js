 ...
 * we get all folders that has a size greater than needed space and find the smallest one of them.
 *
 * @returns { Promise<void> }
 */
const main = async () => {
  Logger.info('Start running 2022 / 07 - part two test');
  ...
  const rootFileSystemSize = 70_000_000;
  const spaceMissing =
    (rootFileSystemSize - spaceNeededForUpdate - folders['/'].size) * -1;

  const result = getSmallFolders(folders['/'], spaceMissing, '>')
    .flat(Infinity)
